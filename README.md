...
- There is no analytics and no error-reporting beacon. The code carries one further, **opt-in** path — an anonymous session-corpus contribution — and the public build ships it **switched off**: no collection endpoint is configured, so no control is rendered, nothing can POST, and the shipped CSP is byte-for-byte unchanged. Where an operator does configure one, it is key-blind by construction, consented per contribution, and refuses outright if the payload contains anything key-shaped. See [`docs/CORPUS-SINK.md`](docs/CORPUS-SINK.md) and its guard ([`test/corpusSink.test.ts`](test/corpusSink.test.ts)).
