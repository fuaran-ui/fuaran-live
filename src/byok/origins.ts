...
// and each adapter carries its own origin – so the CSP and the code that opens
// the connection can never drift apart. That "never" is enforced, not intended:
// test/networkEgress.test.ts asserts this list equals the adapter registry's own
// origin set, and that a key only ever reaches its provider's origin.
//
