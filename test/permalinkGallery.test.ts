...
describe('the gallery', () => {
  ...
  it('emits wire the real strict decoder accepts, already in canonical form', () => {
    for (const { title, wire } of galleryEntries()) {
      ...
      // it is already the canonical form of itself, compared as RAW BYTES with
      // nothing normalised away on either side.
      //
      // Two tier lags have been declared here and both are closed, which is the
      // shape to expect rather than a run of luck: the Chart `"stacked":false`
